...
ls "${master_reads}"/*$r1_tail | tail -n +11 | head -1 > single_update_read_list.txt

start_dir=start_tree_dir
update_dir=update_alignment_dir
single_tax_dir=single_run_update_dir
single_assembly_dir=single_tax_assembly_dir


mkdir $start_dir
mkdir $update_dir
mkdir $single_tax_dir
mkdir $single_assembly_dir
...
for i in $(cat $outdir"/"start_tree_files.txt); do
  ...
  ln -s "${read_dir}"/$i $single_assembly_dir
  ln -s "${read_dir}"/${i%$r1_tail}$r2_tail $single_assembly_dir
done
...
# move the files needed for the single taxon update run into the appropriate folders
for i in $(cat $outdir"/"single_update_read_list.txt); do
  ln -s "${read_dir}"/$i $single_tax_dir
  ln -s "${read_dir}"/${i%$r1_tail}$r2_tail $single_tax_dir
  ln -s "${read_dir}"/$i $single_assembly_dir
  ln -s "${read_dir}"/${i%$r1_tail}$r2_tail $single_assembly_dir
done
...
printf "Full update run speed comparison complete. Beginning single taxon addition run\n"

############################################################################################
# single taxon update run speed comparsion #
############################################################################################

if [ "$tree" != "NONE" ]; then
  cat <<phy_loop > basic_single.cfg
  ...
  read_dir="$outdir/$single_tax_dir"
  ...
  outdir="$outdir/single_tax_phycorder_out"
...
{ time $phycorder_path/multi_map.sh ./basic_single.cfg 2> $outdir/single_tax_update_map_and_tree_times.stderr ; } 2> $outdir/single_tax_update_time.txt

# create a config file on the fly with current settings
cat <<EOF > single_tax_gon_phyling_full_assembly.cfg
## Config file for gon_phyling pipeline
# change the values of the variables to control the pipeline
# the variables are structured like this:
# variable_name="value_of_the_variable"
...
# Path to the directory of reads for assembly
read_dir="$outdir/$single_assembly_dir"
...
{ time $phycorder_path/gon_phyling.sh ./single_tax_gon_phyling_full_assembly.cfg 2> $outdir/single_tax_full_assembly_run_assemly_and_tree_times.stderr ; } 2> $outdir/single_tax_full_assembly_time.txt
